        ...
        try {
            ...
            while (true) {
                ...
                if (header.length() > 10) {
                    ...
                    // multipart/form-data boundary의 body 체크를 위한 index
                    final int boundaryBodyCheckPoint;
                        ...
                        // Multipart/form-data 타입 처리
                            ...
                            for(int i = 0; i<bodyByte.length; i++){
                                if(bodyByte[i]==13 && bodyByte[i+1]==10 && bodyByte[i+2]==13 && bodyByte[i+3]==10){
                                    byte[] boundaryHead = Arrays.copyOf(bodyByte, i+4);     // i+5부터는 body
                                    boundaryBodyCheckPoint = i+5;
                                    String parseBoundaryHead = new String(boundaryHead);
                            ...
                            //TODO: boundary마지막 부분 위치 잡고 그 boundaryHeader부분 이하부터 데이터 읽을 수 있도록 처리
                        }
                    }
                    ...
                    if (headerData.get("Method").equals("GET") && headerData.get("URL").contains("serverRoot")) {
                        ...
                        if (isFile) {
                            ...
                            //TODO: readAllbytes말고 하나씩 읽는거로 실행
                            ...
                            for (int i = 0; i < folderList.length; i++) {
                                if (folderList[i].substring(folderList[i].length() - 1, folderList[i].length()).equals("*")) {
